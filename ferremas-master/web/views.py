...
def contador(request): 
    context={}
    return render(request,'web/contador.html', context)

def administrador(request): 
    context={}
    return render(request,'web/administrador.html', context)

def bodeguero(request):
    context = {
        'pedidos_pendientes': [
            {'id': 101, 'producto': 'Taladro Bosch GSB 550', 'cliente': 'Juan Pérez'},
            {'id': 102, 'producto': 'Pack tornillos Sika x200', 'cliente': 'Ana Martínez'},
        ]
    }
